...
def parse_pdf(
    ...
    for doc_path in doc_paths:
        ...
        try:
            kwargs = (
                {
                    "llmsherpa_api_url": None
                    if host_name is None
                    else f"http://{host_name}:5001/api/parseDocument?renderFormat=all"
                }
...
def _parse_pdf_wrapper(
    args: Tuple[str, str, bool, Optional[str], Optional[logging.Logger]]
) -> List[Document]:
...
if __name__ == "__main__":
    # Load the arguments and the configuration file
    ...
    if len(glob.glob(os.path.join(raw_data_dir, "*", "*", "*.pdf"))) > 0:
        ...
        metadata_table = ddb_resource.Table(
            get_ssm_param_value(
                boto_session,
                get_ssm_param_key(proj_name, stage, "ddb_table", "metadata"),
            )
        )
        ...
        secret = get_secret(boto_session, get_name(proj_name, stage, "docs"))
        ...
        index_name = get_name(proj_name, stage, "docs")
        ...
        proc_data_dir = os.path.join(TEMP_DIR, "proc_data")
        ...
        host_name = None
        if config.pdf_parser_type == "layout_pdf_reader":
            try:
                host_name = get_ssm_param_value(
                    boto_session,
                    get_ssm_param_key(proj_name, stage, "alb_dns", "parsing"),
                )
        ...
        raw_data_dirs = [
            os.path.join(raw_data_dir, category) for category in CATEGORIES
        ]
